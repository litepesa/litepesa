...
#if defined(NDEBUG)
# error "Litepesa cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5f1f2de0cd10a687b8b5b0732db45c20a2c1342e2422eec72ca1d09a86b866fd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Litepesa: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Litepesa Signed Message:\n";
...
{
    // Litepesa: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10); // Litepesa: 10 blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Litepesa: 3 days
static const int64 nTargetSpacing = 2 * 60; // Litepesa: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Litepesa: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Litepesa: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x91522357d51b56d95cb68412efbd6f0b58755a3e6e0d09125ad48e0795c2cb34");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Business Daily 12/Aug/2022 Rich Kenyans save sh2.1bn in dollars daily before polls";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1660324586;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085339013;

        if (fTestNet)
        {
            block.nTime    = 1660324572;
            block.nNonce   = 389861672;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x60dbb73b9f67282c7d56b68ffc63ef2b1779218da1184374620de6d08659ab2a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc6, 0xb4, 0xdf }; // Litepesa: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitepesaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitepesaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitepesaMiner(CWallet *pwallet)
{
    printf("LitepesaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litepesa-miner");
    ...
    try { loop {
        ...
        printf("Running LitepesaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitepesaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitepesaMiner, pwallet));
}
